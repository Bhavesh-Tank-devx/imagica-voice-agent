"""
kaya_branches.py — Pincode / city → Kaya Clinic branch lookup.
Used by the get_closest_branches tool in voice_agent.py.
"""
...
CITY_BRANCHES: dict[str, list[str]] = {
    ...
    # Cities where exact branch address must be confirmed on call
    "Jaipur": [],
    "Lucknow": [],
    "Kochi": [],
    "Indore": [],
...
def get_closest_branches(pincode: str = "", city: str = "") -> dict:
    """
    Return branch information for the given pincode or city name.

    Returns a dict:
      city (str | None): resolved city name
      branches (list[str]): branch areas in that city
      confirm_address (bool): True if agent must confirm address on call
      message (str): agent-ready response describing branches or next step
    """
    ...
    if len(branches) <= 3:
        if len(branches) == 1:
            ...
            listing = f"{branches[0]} and {branches[1]}"
        else:
            listing = f"{branches[0]}, {branches[1]}, and {branches[2]}"
        return {
            "city": resolved,
            "branches": branches,
            "confirm_address": False,
            "message": f"In {resolved}, we have branches at {listing}. Which one is most convenient for you?",
        }
